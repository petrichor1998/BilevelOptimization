...
def create_scaled_data(train, test, scaler):
	from sklearn.model_selection import train_test_split
...
def create_distribution_train_val(dist_list, scaler):
	from sklearn.model_selection import train_test_split
...
def calculate_dist_accuracy(dist_list, model, scaler):
	with torch.no_grad():
		accuracy_list = []
		for d in dist_list:
			X_scaled = scaler.fit_transform(d.values[:, :-1])
