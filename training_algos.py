...
def train_bilevel(epochs, k, tv_list, w_list, model, e1, e2, L, optimizer):
...
def train_bilevel_qform(epochs, k, tv_list, w_list, model, e1, e2, L, optimizer, q):
  for t in range(epochs):
    train_loss = 0
    Ui_loss_params_list = []
...
def train_full_data(epochs, X, y, model, L, optimizer):
  for t in range(epochs):
...
def train_vanilla_qloss(epochs, k, tv_list, model, L, optimizer, q):
  for t in range(epochs):
    train_loss = 0
    Ui_loss_params_list = []
...
def train_full_data_lin_reg(epochs, X, y, model, L, optimizer):
  for t in range(epochs):
    ypred = model(torch.Tensor(X))
    train_loss = L(ypred, torch.Tensor(y).float().unsqueeze(dim = 1))
...
def train_bilevel_lin_reg(epochs, k, tv_list, w_list, model, e1, e2, L, optimizer):
  for t in range(epochs):
    train_loss = 0
    Ui_loss_params_list = []
    ...
    for i in range(k):
      if t == 0 and i == 0:
        ...
        for param in model.parameters():
          param.grad.data.zero_()
      ypred = model(torch.Tensor(tv_list[i][0]))
      L_Ui = L(ypred, torch.Tensor(tv_list[i][2]).float().unsqueeze(dim = 1))
      L_Ui.backward(retain_graph=True)
    ...
    for j in range(k):
      ypred = model(torch.Tensor((tv_list[j][1])))
      val_loss_list.append(L(ypred, torch.Tensor(tv_list[j][3]).float().unsqueeze(dim = 1))) 
  ...
  print(train_loss)
  return model, w_list
def train_bilevel_qform_lin_reg(epochs, k, tv_list, w_list, model, e1, e2, L, optimizer, q):
  for t in range(epochs):
    train_loss = 0
    Ui_loss_params_list = []
    ...
    for j in range(k):
      ypred = model(torch.Tensor((tv_list[j][1])))
      L_Vi = L(ypred, torch.Tensor(tv_list[j][3]).float().unsqueeze(dim = 1))
      qloss_sum = qloss_sum + torch.pow(L_Vi,  q)
...
def train_vanilla_qloss_lin_reg(epochs, k, tv_list, model, L, optimizer, q):
  for t in range(epochs):
    train_loss = 0
    Ui_loss_params_list = []
    ...
    for i in range(k):
      ypred = model(torch.Tensor(tv_list[i][0]))
      L_Ui = L(ypred, torch.Tensor(tv_list[i][2]).float())
      L_Uiq = torch.pow(L_Ui, q)
